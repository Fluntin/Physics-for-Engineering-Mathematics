...
# Calculate the slit positions and resulting intensity patterns for varying number of sources
...
# Enhanced Plot
plt.style.use('seaborn-darkgrid')  # Using seaborn style for a neat and modern look

# Plotting the intensity distribution for the last N sources
plt.figure(figsize=(10, 6))  # Larger figure size for better visibility
plt.plot(screen_positions, intensity_at_screen, color='blue', linewidth=2)  # Blue line with a thicker width
plt.xlabel('Screen position (mm)', fontsize=14)  # Larger font size
plt.ylabel('Intensity', fontsize=14)
plt.title(f"Intensity Distribution for N = {number_of_sources}", fontsize=16)
